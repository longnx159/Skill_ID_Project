...
**Updated:** 07/09/2026
**Purpose:** Independent statistical, manufacturing-data and system-design review before the pilot proposal is reported to management.
...
`Working Hour` = total recorded worker time and may include rework.
...
This metric is not pure first-pass cycle time.

The model may not call it "pure cycle time", "touch time" or "first-pass processing time".
...
**Scrap data is not currently available in the source dataset.**
...
The recovery trajectory model can compute recovery indices from observed PassQty across rounds.
...
> **Not currently feasible.** Requires Scrap data, disposition tracking and severity classification.

## 6.3 Future Scrap fields

When available, the production schema should support:
...
The Recovery formula already reserves a place for Scrap, but production results must remain non-scrap-adjusted until actual data exists.
...
When reliable Scrap data becomes available, use:
...
number of relevant physical parts.

P2:

alignment/sequence dependencies.

P3:

mechanism/function complexity.

The direct PCS proxy remains evidence only.

It must not become an automatic final score.
...
Missing stone information is not zero.

No-stone is zero only when Engineering provides evidence.
...
7. expected time when reliable;
8. workload/capacity when available;
9. Planner review.
...
Phase 4 time model remains diagnostic.

Current historical validation:

* production-only WAPE ≈ 43.53%;
* production + QC feature WAPE ≈ 43.26%.

This does not support operational optimization.

Potential future output:

`{Expected FPY, Expected Cycle Time}`

only after:

* transaction timestamps are available;
* time-model performance is validated;
* rework is appropriately separated;
* speed/accuracy interaction is examined.
...
This is not merely a missing data field waiting for ETL.

Implementing it would require a change to the QC collection process.
...
is now explicitly an **initial candidate envelope**, not a fixed minimum.

It must be finalized only after power analysis.
...
Therefore the final pilot may use more or fewer than:
...
Any illustrative power calculation (e.g., ~5,910 pcs/arm using Base FPY=0.63, Δ=0.04, α=0.05, power=0.80, DEFF=2.65) depends on assumptions about:
...
These are illustrative scenarios using assumed ICC and mean WO size.

**Final sample size must be recalculated from observed pilot-frame ICC and cluster size distribution.**

Do not lock any specific WO count or piece count before the power analysis is completed with observed parameters.
...
Soldering remains exploratory while skill coverage is materially below the approved threshold.
...
Do not design the main pilot in parallel with the lead-in. The lead-in output is a prerequisite for the pilot design.

The illustrative power table (§31.3) is for management communication only; it must not substitute for the lead-in measurement.
...
Assignment randomization/blocking must prevent selective exposure based on expected outcome.

## 32.1 ITT and PP analysis

The pilot must report both:

### ITT (Intent-to-Treat) — **Primary**

Compare outcomes based on whether Skill ID recommendations were **available** to the Planner, regardless of whether the Planner followed them.

This answers:

> "Does making Skill ID available improve production outcomes?"
...
> "When Planners follow the recommendation, does it produce better outcomes?"

ITT must be the primary analysis because PP is subject to selection bias: Planners may follow the model only when they already believe it is correct.

## 32.2 Blinding

Worker-level blinding is desirable but not required.

Randomization and standardized operating procedure are the primary protections against bias.

The critical requirements are:

* Worker does not self-select treatment;
* treatment assignment does not depend on expected outcome;
* process/time conditions are blocked;
* contamination is measured and reported.
...
Before Scrap data exists, the pilot must explicitly mark Scrap as:
...
The prospective pilot compares:
...
on comparable Semi/Process/time blocks.
...
Planner agreement alone is not proof of model effectiveness.
...
| Time model                | Future-period WAPE                       | Diagnostic until approved threshold           |
| Scrap                     | Future once data exists                  | Validate independently; never assume zero     |

---

# 36. Validation sample-size requirement

Before any pilot metric is declared Pass/Fail, define:
...
should be treated as a universal acceptance threshold without power justification.

Likewise, no calibration result with tiny sample sizes may be interpreted as meaningful merely because the numerical metric looks favorable.
...
3. Reliable transaction timestamps.
...
> current QC process does not capture it.

It is a future process-design enhancement.
...
must not override the canonical Size Adjusted model.
...
Probability thresholds are business-policy decisions, not statistical defaults.
...
* `λ_scrap` calibration parameter and penalty logic (§19) when Scrap data becomes available.

This prevents silent double-penalization.
...
**Defect Code / severity weighting is not a current pilot approval item** because the QC process does not capture it.

If QC wants severity weighting later, that becomes a separate process-change project.
...
If Process-specific Verified Skill coverage is below the agreed minimum, proposed initial threshold:
...
and may not drive production assignment decisions.

Soldering currently falls into this category.
...
| Scrap effect: Tier 3 (Loss model) | 🔵 Not feasible; requires Scrap + disposition + severity   |
...
The historical allocation mechanism must be documented and its relationship to estimated difficulty must be tested.

Compute |r| between mean assigned-worker skill and observed group FPY per Process. If |r| exceeds the Engineering/Planner-approved threshold → flag entire Process as `Assignment-Conditional`.

The pilot can validate whether the recommendation improves outcomes, but historical confounding is not assumed to disappear automatically.

### B2 — Worker/group identifiability and connectivity

**Connectivity diagnostics must be completed BEFORE fitting a production GLMM.**

Build bipartite graph Worker×Group per Process. Compute connected components via union-find. For each component report: `ComponentSize`, `AnchorAvailability` (worker skill in [4.5, 5.5]), edge density, `ConnectivityStatus`.

Suggested threshold (Engineering/IT approved): component < 30 observations OR no anchor → `WeakNetwork`, no calibrated v_j, fallback Raw FPY + Bayesian empirical shrinkage toward Process mean.

Convergence alone is insufficient. Fit all data; do not discard disconnected components.

Output: one summary row per Process — sufficient for immediate Go/No-go decision on v_j publication.

### B3 — Within-WO overdispersion

The selected observation model must demonstrate appropriate treatment of intra-WO dependence.

Beta-Binomial captures extra-binomial variation but does not identify its physical source. Compare Model A vs Model B using WAIC/LOO-CV; when tied, choose the more parsimonious model (§9.4).

### B4 — Size invariance

Size Adjusted pooling must pass residual size-effect screening.

Preferred: cluster-bootstrap or permutation test. Fallback: effective-N adjusted z. Engineering decides splits based on `SizeResidualScore`, `SizeResidualN`, `SizeResidualMethod`, `SizeResidualFlag`.

### B5 — Missing skill, especially Soldering

Candidate missing-skill methodologies must be compared on high-coverage Processes first, then applied to Soldering.

Soldering remains exploratory until sufficient skill coverage (≥60%) and calibration are achieved **on Soldering data specifically**, not inferred from other Processes.

### B6 — Recovery data integrity

FirstFailQty=0, round continuity and quantity reconciliation must be enforced automatically.

Recovery outputs must be labeled as Observed Recovery Difficulty. Missing pieces are not assumed to be scrap.

Recovery sensitivity bounds (§18.4) must be computed for every group with open WOs. Groups exceeding the sensitivity threshold are flagged `RecoveryUnstable` and excluded from production ranking.

### B7 — Leakage

No WO lifecycle may cross Train/Test boundaries.

**All random effects (u_k and v_j) must be estimated from Train cohort only (§15.5).** Never refit on combined Train+Test then split to evaluate.

Automated assertion: no worker_id or group_id in the estimation set has evaluation-cohort observations used in parameter estimation.
...
Any illustrative calculation (e.g., ~5,910 pcs/arm at ICC=0.15, m=12) is a scenario, not a locked requirement. Do not design the main pilot before the lead-in is complete.

### B9 — BOM data quality

758 "Cannot Find Data" entries and 26 duplicates must be resolved by Engineering before BOM-derived factors (PartMechanism, MaterialDesignProcess) enter production.

Before BOM sign-off:

* PartMechanism and MaterialDesignProcess may only be scored via manual Engineering rubric.
* Evidence source must be labeled `Manual`, not `BOM`.
* Minutes field must not be summed until Engineering confirms per-operation vs cumulative semantics.
* Unit casing must be standardized via a versioned lookup table.
...
v0.5.2 is methodologically sufficient to proceed to a controlled pilot, provided that:

1. The pilot treats recovery as an **observed-recovery** signal, not as true recovery difficulty.
...
3. Missing Scrap is never assumed to equal zero.
...
* Scrap is not yet modeled because actual Scrap data is unavailable.
...
| 3 | Full quality-loss model | Required | 🔵 Not feasible yet |

Future model versions may introduce:

* Scrap-aware recovery;
* survival/censoring models;
* defect severity;
* dynamic skill;
* cold-start Bayesian priors;
* speed–accuracy optimization.

These enhancements must be independently validated before affecting production decisions.
